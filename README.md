...
To obtain a good data-based model, all frequencies and amplitudes within the desired operating range of this nonlinear dynamical system needs to be excited. An **A**mplitude-modulated **P**seudo **R**andom **B**inary **S**equence, generated by `GenSkyline` function in [ `Files/ExcitationSignal.m` ]( https://github.com/JohnDoe2576/MagLevNet/blob/master/Files/ExcitationSignal.m ), helps design such a signal. This excitation signal is supplied to armature coil of the electromagnet, and the corresponding position ( vertical ) of the levitating magnet is simulated using MATLAB `ode45` solver ( refer function [ `SimMagLev` ]( https://github.com/JohnDoe2576/MagLevNet/blob/master/SimMagLev.m ) ) to obtain the input-output sequence. The figure below shows the an example of input APRBS sequence ( U ) and corresponding system output ( Y ).

![]( https://github.com/JohnDoe2576/MagLevNet/blob/master/DataStore/Fig/MagLevTestingData1.png )

### Setting-up a Neural Network
A Dynamical Neural Network ( Multi-Layer Perceptron ) architecture is chosen such that the current output of system, Y [ k ] is a function of previous inputs to the system ( U [ k-1 ], U [ k-2 ], . . ., U [ k-l ] ) and previous outputs from the system ( Y [ k-1 ], Y [ k-2 ], . . ., Y [ k-m ] ). Essentially, these **previous inputs to the system ( U's )** and **outputs from the system ( Y's )** form **inputs to the Neural Network**. The network is then constructed with the following parameters:
> - Number of Hidden Layers: 1
> - Number of neurons in Hidden Layer: 10
> - Number of delayed inputs: 8
> - Number of delayed outputs: 8

The well-trained network should be general enough ( i.e.; should not overfit ) to accuratley predict any behaviour outside of the training set ( but, within the trained parameter range ). One of the methods used by MATLAB Neural Network Toolbox [ **Ha2014** ] to avoid *over-fitting* is termed *Early Stopping*. In this method, the training data-set is divided into three parts, namely *Training, Validation & Testing*. Each of these divisions must necessarily be well-correlated with each other for proper results. The training part is used solely for training ( obtaining optimal weights and biases ) the network. At each training iteration, the network performance ( in terms of **M**ean **S**quared **E**rror ) is assessed in its capability to predict the data in validation part. Training iterations are continued till MSE of training part improves and MSE of validation part worsens ( which is an indication of over-fitting ). Finally, the testing part is used to check network performance in predicting data that had no role in training phase.
